...
# Procedure to parse a value from a report file using a regex
# proc parse_report_value {report_file pattern {default_val "N/A"}} {
#     if {![file exists $report_file]} {
#         puts "WARNING: Report file not found for parsing: $report_file"
#         return $default_val
#     }
#     set fh [open $report_file r]
#     set value $default_val
#     # Initialize match variable to avoid errors if regexp doesn't match on first line
#     set extracted_value "" 
#     while {[gets $fh line] >= 0} {
#         if {[regexp -- $pattern $line match extracted_value]} {
#             set value $extracted_value
#             break
#         }
#     }
#     close $fh
...
proc setup_and_compile_design {algorithm operation RESULTS_DIR_ABS_PATH stats_file_handle create_script_full_path} {
    ...
    # --- 2. Compilation Phase ---
    # Reset runs to ensure a clean state
    reset_runs synth_1
    reset_runs impl_1

    puts "INFO: Starting Synthesis for $proj_name..."
    if {[catch {launch_runs synth_1 -jobs 4} synth_msg]} {
    ...
    if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
        puts "ERROR: Synthesis failed for $proj_name. Status: [get_property STATUS [get_runs synth_1]]"
    ...
    if {[catch {launch_runs impl_1 -to_step write_bitstream -jobs 4} impl_msg]} {
    ...
    if {[get_property PROGRESS [get_runs impl_1]] != "100%" || [string is true [get_property NEEDS_REFRESH [get_runs impl_1]]]} {
        puts "ERROR: Implementation/Bitstream failed for $proj_name. Status: [get_property STATUS [get_runs impl_1]]"
    ...
    open_run impl_1 ;# Ensure post-implementation design is open for reporting
    ...
    # --- 5. Cleanup Phase ---
    # if {[catch {close_project -quiet} close_err]} {
    #     puts "WARNING: Failed to close project $proj_name: $close_err"
    # }
...
set RESULTS_DIR_NAME "BScThesis.results"
set RESULTS_DIR_ABS_PATH [file join $WORKSPACE_ROOT $RESULTS_DIR_NAME]

# Create results directory if it doesn't exist
if {![file isdirectory $RESULTS_DIR_ABS_PATH]} {
    if {[catch {file mkdir $RESULTS_DIR_ABS_PATH} mkdir_err]} {
        puts "ERROR: Could not create results directory at $RESULTS_DIR_ABS_PATH: $mkdir_err"
        return 1
    }
    puts "INFO: Created results directory: $RESULTS_DIR_ABS_PATH"
} else {
    puts "INFO: Results directory already exists: $RESULTS_DIR_ABS_PATH"
}
